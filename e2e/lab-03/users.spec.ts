...
test('admin creates, edits, deactivates and resets an account; user must change password',async({page},info)=>{
  ...
  try{
    ...
    await page.screenshot({path:`artifacts/lab-03/issue7-${info.project.name}-users.png`,fullPage:true});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);
    if(info.project.name==='desktop'){await page.setViewportSize({width:820,height:1180});await page.screenshot({path:'artifacts/lab-03/issue7-tablet-users.png',fullPage:true});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);await page.setViewportSize({width:1440,height:1000})}
    ...
    await page.screenshot({path:`artifacts/lab-03/issue7-${info.project.name}-edit.png`,fullPage:true});expect(await page.evaluate(()=>document.documentElement.scrollWidth<=window.innerWidth)).toBe(true);
